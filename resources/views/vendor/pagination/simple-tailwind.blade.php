@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex flex-col">
        <div class="self-center mb-6 w-full text-center">
            <h2 class="text-lg dark:text-gray-300 text-center">
                Showing results from {{ $paginator->firstItem()}} to {{ $paginator->lastItem() }}
            </h2>
        </div>
        ...
        <div class="flex justify-between">
            ...
            @else
                <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-600 bg-white border-2 border-gray-200 cursor-default leading-5 rounded-md dark:focus:ring-blue-300 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-700 dark:hover:text-blue-300 dark:hover:border-blue-300 dark:focus:text-blue-300">
                    {!! __('pagination.next') !!}
                </span>
            @endif
        </div>
